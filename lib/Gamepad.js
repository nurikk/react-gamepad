...
var Gamepad = function (_React$Component) {
  ...
  function Gamepad(props, context) {
    ...
    for (var button in layout.buttons) {
      _this.padState.buttons[button] = false;
    }

    for (var axis in [].concat(_toConsumableArray(layout.axis), _toConsumableArray(layout.buttonAxis))) {
      if (axis && !axis.startsWith('_')) {
        _this.padState.axis[axis] = 0.0;
      }
    }
    return _this;
  }
